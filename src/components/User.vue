<template>
  <div>
    ...
    <el-card class="box-card">
      <!--v-model:双向数据绑定  clearable:输入框可清空  @clear清空后回调  @click点击搜索按钮搜索数据-->
      <el-row :gutter="20">
        <el-col :span="8">
          <el-input
            placeholder="请输入搜索内容"
      ...
      <el-table :data="userInfos" border style="width:100%">
        ...
        <el-table-column prop="mg_stale" label="状态" width="70">
          <el-switch
...
export default {
  ...
  methods: {
    ...
    async showEditDialog(id) {
      ...
      if (res.meta.status !== 200) {
        return this.$message.error(res.error.msg)
      }
      this.editForm = res.data
    },
    delUser(id) {
      this.$confirm('确定删除用户吗?', '删除', {
